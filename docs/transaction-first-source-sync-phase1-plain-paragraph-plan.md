...
- Current application version: `0.13.133`
...
- Current structure ownership: exact single-list-subtree and existing fenced-code-content journals may publish transaction bytes; unsupported families remain legacy fallback
...
The default application still publishes ordinary paragraphs through legacy unless the explicit authority gate is enabled. Shadow, authority, 1000-paragraph BOM/CRLF, list subtree callback/forced-flush, existing code-block-content callback/forced-flush, 39/39 source probes and adjacent high-risk list/input-rule UI matrices are green. Code-block attrs/fence structure, table, quote and other unported structural journals remain outside this plain-paragraph authority family.
