...
- 源码/富文本一致性最终收口的唯一长期进度账本是 [`source-rich-consistency-completion-plan.md`](./source-rich-consistency-completion-plan.md)。后续阶段、完成条件、门禁、提交边界和正式安装包 P0 关闭标准都以该文档为准；不要从聊天摘要或 `/tmp` 状态文件推断完成度。阶段 A 已由本地提交 `9dafd76` 完成；阶段 C 的四个 Blockquote legacy owners 已由 `5da0e17` 完成；0.13.150 非空 `code_block → paragraph` 已由 `0614893` 收口；0.13.151 interior empty-item 已由 `c70fa0b` 收口；0.13.152 plain tail empty-item 已由 `8eed3a2` 收口。当前阶段 E 已推进到 0.13.165：0.13.154 isolated empty ordered lift、0.13.155 RS-72 single-successor、0.13.156 multi-successor relabel chain、0.13.157 nested plain bullet tail-empty Tab sink、0.13.158 plain nonempty bullet middle/tail Tab sink、0.13.159 single nested child Shift+Tab outdent、0.13.160 multi-child last-item Shift+Tab outdent、0.13.161 multi-child first-item Shift+Tab outdent、0.13.162 nested plain bullet middle/end Enter split、0.13.163 nested plain bullet sibling Backspace join、0.13.164 task checkbox toggle、0.13.165 task end-Enter empty sibling split 均已独立 transaction-owned。task Enter 第一拍已迁移；`empty-task-sentinel-filled`、task empty-row Backspace、conversion、input rule 与跨列表/coalescing 仍未完成，不能把 List legacy 写成已全部退役。
- 当前分支：`fix/rs-41-rich-source-divergence`
- 当前源码与 `package-lock` 为 `0.13.165`；本轮仍是 transaction-journal 架构迁移。已为朋友测试生成 **未签名** 的 0.13.165 arm64/x64 DMG/ZIP，但没有安装覆盖 `/Applications/HorseMD.app`、没有 notarize/publish；因此该外发包只用于功能试测，不等于阶段 I 正式安装包资格验收。`/Applications/HorseMD.app` 与旧人工 trace 实例仍不能用于验收 0.13.132–0.13.165 的 journal owners，后续正式手测必须从当前已提交源码重新 `dist:dir`、覆盖安装并带 `--horsemd-input-trace` 启动。旧运行 trace 路径历史记录仍为 `/var/folders/4y/k4t_v1r1745gl5m_h1vwc6j40000gn/T/horsemd-input-trace-16298.jsonl`；后续需要用户手测时必须从当前源码重新 `dist:dir`、安装并以 `--horsemd-input-trace` 启动后再更新此项。
