...
- 源码/富文本一致性最终收口的唯一长期进度账本是 [`source-rich-consistency-completion-plan.md`](./source-rich-consistency-completion-plan.md)。后续阶段、完成条件、门禁、提交边界和正式安装包 P0 关闭标准都以该文档为准；不要从聊天摘要或 `/tmp` 状态文件推断完成度。当前阶段 A：把 0.13.148 的 code-block exit + 首批 legacy退役整理成可复现本地检查点，并明确排除 paragraph/code、boundary join等下一阶段草稿。
