...
- 源码/富文本一致性最终收口的唯一长期进度账本是 [`source-rich-consistency-completion-plan.md`](./source-rich-consistency-completion-plan.md)。后续阶段、完成条件、门禁、提交边界和正式安装包 P0 关闭标准都以该文档为准；不要从聊天摘要或 `/tmp` 状态文件推断完成度。阶段 A 已由本地提交 `9dafd76` 完成；阶段 C 的四个 Blockquote legacy owners 已由本地提交 `5da0e17` 在 `0.13.149` 完成退役，并通过正反、兼容、全局门禁与post-commit smoke。当前返回阶段 B，下一项审计非空 `code_block → paragraph`，随后处理 boundary join 和其余 fence lifecycle；`paragraph → code_block` 右键草稿当前没有产品可达的 `BLOCK_TYPES` 入口，不能因测试草稿存在就计入完成范围。
