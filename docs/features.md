...
- 大纲（`Ctrl+Shift+L`）：从内容解析标题，点击跳转，随编辑实时更新（`Outline.jsx`）。标题可折叠/展开，顶部只有一个类似文件树的总开关；折叠包含当前标题的父级时，父级保持高亮提示，避免点击无反馈。
...
- 光标映射不依赖关键词：富文本 selection 与源码 textarea selection 通过 Markdown raw offset / ProseMirror position 映射，图片、链接、重复文本和大文档都比纯文本锚点稳定。
