...
- 大纲（`Ctrl/Cmd+Shift+L`）：从内容解析标题，点击跳转，随编辑实时更新（`Outline.jsx`）。标题可折叠/展开，顶部只有一个类似文件树的总开关；默认保留前两层真实目录层级（H1/H2/H3 显示 H1+H2，全 H1 文档则全部显示），折叠包含当前标题的父级时，父级保持高亮提示。源码模式使用标准 Markdown AST 解析标题，避免代码块、Setext、转义或行内格式造成两种模式目录不一致。
...
6 套配色：暖光、暖夜、莫兰迪·灰绿 / 豆沙 / 雾蓝 / 暮。右下角状态栏带色块的主题选择器；`Ctrl/Cmd+Shift+T` 循环切换。
...
**实现**：
- `editor-api.js` 的 `getPdfSource()` 克隆当前 ProseMirror 视图并生成干净 HTML 和标题列表：移除编辑器控件，把 CodeMirror 代码块压平成 `<pre><code>`，并清理临时属性。
