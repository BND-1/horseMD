...
**实现**：`Sidebar.jsx` + 主进程 `filesystem.js` 的 `fs:readDir` / `fs:duplicate`，以及 `watchers.js` 的 `watch:start`（chokidar 监听文件夹，去抖后推 `watch:changed`）；文件夹启动参数见 `main/index.js` 的 `extractArgs()`（区分文件 vs 目录，目录走 `open-folder`）
