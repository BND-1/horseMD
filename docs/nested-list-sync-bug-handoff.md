...
> 状态：2026-08-09 已完成代码修复并在 `0.13.29` 发布候选上重新跑完整家族矩阵；当前仍未正式发版。
...
### 4.4 `U+200B + 前导空格` 列表转换误拒绝（0.13.27）

真实文件 `11111.md` 的列表项包含 HorseMD/Typora 式前导空格哨兵：作者源码是
`U+200B + 5 个 ASCII 空格 + 正文`，remark canonical 则写成
`&#x20; + 4 个 ASCII 空格 + 正文`。两者反转义后完全相同，但旧
`patchConvertedListMarkers()` 直接比较 canonical 拼写，因此把一次只改 marker 的
无序→有序转换误判为正文也发生变化，并以 fail-closed 提示拒绝转换。

修复只作用于**比较视图**：列表 marker 去除后先经过 `canonicalTextToSource()`，再做
标点反转义比较；真正输出仍只替换目标 marker，作者的 `U+200B`、空格数量、正文、
其他层级和其他列表完全不动。fixture 与纯函数/UI 回归都按逐字节断言保留
`U+200B + 5 spaces`，不能用 trim 后的可见结果代替。
