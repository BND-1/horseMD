...
| E | 退役 list legacy owners | **进行中：0.13.151–0.13.158 已完成 empty-item/ordered successor Backspace 链 + plain bullet empty-tail/nonempty middle-tail Tab indent；Shift+Tab outdent、nested split/join 仍待拆 family** | list subtree、item text、Enter/Backspace、task、input rule、conversion 分 family退役 |
...
- 阶段E仍未完成。已用原生`liftListItem`比较证明outdent至少有三种物理拓扑：**single nested child**为单`ReplaceAroundStep(sliceSize=0)`；**first of multiple nested children**为`ReplaceAroundStep + ReplaceStep`两Step；**last of multiple**为单`ReplaceAroundStep(sliceSize=3,openEnd=1)`。下一步应先从single-child Shift+Tab outdent做真实Electron取证并单独成family，不能把三种路径塞进一个宽owner；nested split/join继续排后。
