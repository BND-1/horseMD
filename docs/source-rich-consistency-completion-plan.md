...
- [ ] **P3 Nested bullet IME→Enter 并回已有 split family**：先用 P1 将 terminal Enter 前的 pending text原子映射回 source，再由既有 split topology证明最终结构 Step；empty nested baseline没有正文 source-map时，用已证明 parent/相邻物理 list row作为稳定锚点定位唯一 authored empty nested marker，不能伪造 offset。
  ...
  - **P5c 未做——格式分歧文档的跨块选区删除**：框选跨多块（含 fence）大段删除 → 0.13.192 后不再警告，但删除靠后续回调/flush 重试累计 delta，**不即时同步**。根因：文档与 canonical 只有格式分歧（紧凑表格分隔行 vs 对齐填充、`45~60` vs `45\~60` 转义）→ 可见流完全一致 → `preserveDivergedVisibleDelete` 的 `prevVis === srcVis` 守卫正确拒绝；无 owner 拥有此形状。修法 = 以选区 ReplaceStep from/to（PM 事务证据有精确范围）为锚的选区级 owner，在源码定位对应区间（含不可见 fence 行）整段删除，过完整校验。与 P5b 同属"选区级结构操作"家族，可一并设计。
