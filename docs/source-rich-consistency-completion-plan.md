...
- [ ] **P3 Nested bullet IME→Enter 并回已有 split family**：先用 P1 将 terminal Enter 前的 pending text原子映射回 source，再由既有 split topology证明最终结构 Step；empty nested baseline没有正文 source-map时，用已证明 parent/相邻物理 list row作为稳定锚点定位唯一 authored empty nested marker，不能伪造 offset。
  ...
  - **P6e 未做（下一会话最高优先级，用户硬性要求"以后改什么功能都不能再触发"）——松散列表项续行内的 IME 结构性段落分裂 owner（trace-38723 2026-09-12 10:16:37）**：用户在 333K redis 大文档（394 fence + 642 列表行 + 既有转义/表格填充分歧态）的列表项**缩进续行段落**（`- **返回值：**` 项下的 `  执行加法操作之后…` 段）用中文 IME 连续输入，composition 中途夹了一次**结构性段落分裂**（journal `ReplaceStep from=15508 structure:true sliceSize=4`）。三层后果：① 文字变更被 `locally-aligned-change` 成功发布，但段落分裂无任何 owner/mapper 认领——PM 比源码**多出顶层结构块**（canonical 3540 vs source 3535 顶层，首个列表路径 972→973 错位）；② 下一键校验抓住漂移：`listSlotsMatch=false` → `source-list-structure-mismatch` fail-closed 警告（源码未被写坏 ✓，编辑器内容完好 ✓，保存写盘走 PM 序列化内容安全 ✓）；③ 用户 4 次 Backspace 后重试 `visible-stream-mismatch` → 同步挂起。**已排除**：0.13.207 的性能缓存改动与本事故无关（离线逐字节复现：生产端与离线端 `preserveRichMarkdownSource` 产出完全一致的 `locally-aligned-change`/333602 字节候选——见 `scripts/fixtures/ime-loose-item-split/repro.mjs`）。**证据已永久化**：`scripts/fixtures/ime-loose-item-split/`（source/previous-canonical/canonical 三元组逐字节 + journal.json 完整 evidence dump + repro.mjs，Node 直跑 ~775ms）。**修法（focused owner）**：松散项续行段落分裂是源码可表示形状——终态 bounded patch = 在续行段落的分裂点插入作者 EOL + 项缩进（`  `）续行；合同 = pending-text 链（P1 已有）+ 终态结构 split 落在已证明的松散 list_item 续行 paragraph 内；镜像 P3b（嵌套 bullet IME→Enter split）的链重放 + pre-split doc 形状匹配。**验收（用户的"不能再触发"）**：① 新 owner Node 合同（正例=trace 形状 + 负例 fail-closed）；② 真实 Electron E2E 用真 IME（`Input.imeSetComposition`）在真文档形状上复刻 composition→split→继续输入→零警告、源码含新续行、存盘冷重开一致；③ **加入常驻矩阵**（goal-matrix 或 family-matrix 增加该场景），使今后任何功能/修复改动都会跑到它——这是"以后改什么都不能再触发"的机制化保障；④ 相邻回归：cross-fence-span、list-subtree、blockquote 家族、mixed、goal-matrix。
