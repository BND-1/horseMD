...
- [ ] **P3 Nested bullet IME→Enter 并回已有 split family**：先用 P1 将 terminal Enter 前的 pending text原子映射回 source，再由既有 split topology证明最终结构 Step；empty nested baseline没有正文 source-map时，用已证明 parent/相邻物理 list row作为稳定锚点定位唯一 authored empty nested marker，不能伪造 offset。
  ...
  - **P7b 已完成（2026-09-13 凌晨，0.13.210，trace-62663 用户实机触发驱动）——分歧大文档 transition 等价性墙 + 两个伴生根因，三层按层修复**：① transition 通道内联文本化（`semanticJson` 新增 `inlineTextual` 选项，仅 `semanticTransition` 启用：link/strong/emphasis 摊平、去 marks、合并相邻文本；两侧对称，块结构+可见文本仍严格；合同 `test:transition-inline-textual` 7 条）；② `diverged-visible-delete` 边界镜像（canonical 删除跨行与否决定 end-affinity，单行删除断言 raw 范围无换行——修掉吞 ```bash 围栏的损坏候选）；③ `empty-paragraph-before-fence-removed` 加入项内尾空段折叠许可（joinBackward 消耗空 sibling 后的不可编码 transient）。验证：`test:redis-joinbackward-replay-ui` 全链重放（分裂→IME 填充→删 `===`→逐字删→删空项→joinBackward→继续打字，零警告+磁盘字节）；line1-replay、ime-loose E2E、完整 preservation、goal-matrix 复绿。**方法论沉淀**：带 trace 会话的 `markdown-sync-integrity` 事件自带失败四元组全字节，行级 diff 对称性两分定位（mapper 产坏候选 vs 证明层误拒）；E2E 事故重放会持续挖出相邻层 bug（本轮 ②③ 都是重放首轮/次轮暴露），重放到全绿才算家族关闭。
