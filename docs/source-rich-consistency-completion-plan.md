...
- [ ] **P3 Nested bullet IME→Enter 并回已有 split family**：先用 P1 将 terminal Enter 前的 pending text原子映射回 source，再由既有 split topology证明最终结构 Step；empty nested baseline没有正文 source-map时，用已证明 parent/相邻物理 list row作为稳定锚点定位唯一 authored empty nested marker，不能伪造 offset。
  ...
  - **P7b 未做（trace-62663 第三族，2026-09-12 晚实机归因完毕）——分歧大文档的 transition 等价性墙**：用户在 redis 文档连续删除（嵌套项删字 → 空项删除 → joinBackward 结构步 + 后续打字并入同一 journal）触发 `source-document-mismatch` 重复警告。机制：`ok = semanticProofOk && listProofOk`，listSlotsMatch=true；semanticOk=false 是慢性（作者字节 vs 序列化字节的内联解析不对称：autolink 括号、strong 拆分，全文数十处）；分歧文档唯一通过通道 transitionOk 的窗口比对仍内联形态敏感——变更窗口触碰不对称位即失败。已排除 checkpoint 投毒（coordinator 发布后重建 trust）与 PM 积累空段（顶层归一化滤除）。修法方向与验收见 `docs/handoff-transition-equivalence-wall.md`；证据 `scripts/fixtures/redis-transition-wall/`。
