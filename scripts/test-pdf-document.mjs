...
assert.ok(defaultCss.includes(':root { --hm-pdf-font-size: 11pt;'))
assert.ok(defaultCss.includes('--hm-pdf-line-height:1.75;'))
assert.ok(defaultCss.includes('font-size: var(--hm-pdf-font-size, 11pt)'))
assert.ok(defaultCss.includes('table-layout: auto'))
assert.ok(defaultCss.includes('table[data-hm-pdf-table-layout="measured"] { table-layout: fixed; }'))
assert.ok(defaultCss.includes('.doc th > p, .doc td > p {'))
assert.ok(defaultCss.includes('margin: 0; padding: 0; line-height: inherit;'))
assert.ok(defaultCss.includes('overflow-wrap: anywhere'))
assert.ok(defaultCss.includes('.doc tr, .doc th, .doc td { break-inside: auto'))
...
const fontSizeCss = buildPdfCss({ fontSizePt: 14 })
assert.ok(fontSizeCss.includes('--hm-pdf-font-size: 14pt'))
assert.equal(normalizePdfOptions({ fontSizePt: 7 }).fontSizePt, 8)
assert.equal(normalizePdfOptions({ fontSizePt: 30 }).fontSizePt, 24)
assert.equal(normalizePdfOptions({ fontSizePt: 'invalid' }).fontSizePt, 11)
