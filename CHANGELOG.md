...
### Added
- **标题间距设置（[#96](https://github.com/BND-1/horseMD/issues/96)）** — “设置 → 编辑器”和底部“排版”面板新增独立标题间距控制，可在紧凑、较紧、标准和宽松档位之间切换并细调；只改变 H1–H6 周围留白，不连带修改正文、列表或代码块间距。
...
### Fixed
- **行内代码输入（[#93](https://github.com/BND-1/horseMD/issues/93)）** — 输入空反引号对后可连续输入行内代码，编辑期间临时显示成对反引号，光标离开或输入闭合反引号后再收起；连续输入三个及更多普通反引号仍保持原样，编辑装饰不会进入 Markdown 源文件。
