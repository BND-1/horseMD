...
### Added
- **新增华为 VRP 代码块语言与命令补全** — 代码块语言选择器新增「VRP」，` ```vrp ` / ` ```huawei ` 围栏按华为 VRP 配置语法高亮：`system-view`、`interface`、`undo` 等命令、接口名（`Vlanif10` / `GigabitEthernet0/0/1`）、IPv4 与 MAC 地址、`#` 注释、`[Huawei-…]` 视图提示符分别着色。在 VRP 代码块内输入命令开头（`sys` / `int` / `acl` / `vlan` …）会弹出命令补全，`Ctrl+Space` 显示全部命令；内置 `vlan-access` / `vlan-trunk` / `vlanif` / `static-route` / `ospf` / `ssh` / `acl` / `dhcp` 八个多行配置片段模板，插入后自动选中待填参数（接口编号、VLAN、IP、密码等），直接输入即可替换。补全通过语言的 `LanguageSupport` 挂载，因此只在 VRP 代码块内生效，其它语言代码块行为不变；斜杠菜单新增 `/vrp` 可直接插入 VRP 代码块。语言模块为手写 StreamLanguage（VRP 无现成 CodeMirror 语言包），补全模块经动态 `import()` 懒加载，不影响编辑器首屏。新增纯 Node 回归 `scripts/test-vrp-language.mjs`（tokenizer 断言 + 补全表契约 + 过滤排序）。
