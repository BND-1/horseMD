...
## 当前 Issue 分流（2026-07-21）

以下状态以公开 Issue 和当前 `main` 的提交为准，避免把已完成的修复、待产品决策的需求和平台专项混在同一个开发队列里。

### 已完成，等待随下一版本交付

- **#80**：收紧代码块上下留白。
- **#81**：自定义 CSS 片段、局部预览与元素检查入口。
- **#83**：桌面右侧悬浮章节导航；点击章节后鼠标离开即收起，不再要求点击正文才关闭（`4d76cd0`）。
- **#85**：字体选择器搜索体验。
- **#86**：表格编辑、保存回读、宽表横向滚动、行列操作与长按实时列宽拖动。

### 已部分完成，继续保持边界

- **#63 自定义快捷键**：第一期命令注册、设置录制、冲突校验和首批应用/块命令已可用。H5/H6 会保留用户原有大小写（`2b31d93`）。粗体、公式、表格和 CodeMirror 内部快捷键仍不开放自定义：先为每一个命令建立稳定 API 和专门回归，再逐项加入，不能用全局 `keydown` 覆盖编辑器生命周期。
...
- **#76 移动端 WebDAV/S3**：桌面手动同步已独立闭环；移动端保持关闭，直到完成 Keychain/Keystore 凭据存储、原生 HTTP、Capacitor 文件适配器、后台/网络失败语义和真机 WebDAV/S3 回归。详见 [移动端同步架构](./docs/mobile-cloud-sync-architecture.md)。
- **#23 Android 外部目录**：需要 Android Storage Access Framework（SAF）目录授权和持久 URI 权限，不能用当前 app-private 文件系统伪装为“打开文件夹”。
