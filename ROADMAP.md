...
自定义快捷键来自 [Issue #63](https://github.com/BND-1/horseMD/issues/63)，计划在统一命令系统后实施，不在各组件里继续追加零散 `keydown` 判断：

1. **统一命令注册表**：为应用命令和编辑器命令定义稳定 ID、默认快捷键、作用域与执行入口。
2. **设置页录制**：直接按键录入组合键，支持清除、单项恢复和全部恢复默认。
3. **冲突与保留键校验**：阻止重复绑定以及覆盖复制、粘贴、撤销、退出等系统关键组合。
4. **跨平台显示**：同一配置在 macOS 显示 `Cmd`，在 Windows/Linux 显示 `Ctrl`；菜单提示、工具栏和命令面板同步更新。
5. **首批可配置动作**：文件操作、命令面板、PDF、查找、侧边栏、大纲、源码模式、粗体/斜体/高亮、标题、代码块、行内代码和行内/块级公式。
6. **双层分发**：应用级快捷键与 Electron 菜单保持单一触发；编辑器级快捷键通过 ProseMirror/CodeMirror 正确处理选区和焦点，避免一次按键执行两次。

浏览器式 PDF 打印中心涉及的 `Ctrl/Cmd+P` 与命令面板冲突，将在这套注册表中解决；默认键位与迁移策略在实现打印中心时一起确定。
