...
export const LIST_NESTED_NONEMPTY_BULLET_INDENT_TRANSACTION_FAMILY =
  'list-nested-nonempty-bullet-indent'
export const LIST_NESTED_NONEMPTY_BULLET_INDENT_TRANSACTION_BOUNDARY =
  'transaction-list-nested-nonempty-bullet-indent'
...
const classify = ({ journal, expectedDoc }) => {
  if (!journal?.oldDoc || !expectedDoc || !Array.isArray(journal.entries)) {
    return rejected('nested-nonempty-bullet-indent-document-missing')
  }
  const before = topLevelSourceSyncEntries(journal.oldDoc)
  const after = topLevelSourceSyncEntries(expectedDoc)
  if (before.length !== after.length) return rejected('nested-nonempty-bullet-indent-top-level-count')
  const changed = before
    .map((entry, index) => entry.node?.eq?.(after[index]?.node) === true ? null : index)
    .filter((index) => index != null)
  if (changed.length !== 1) return rejected('nested-nonempty-bullet-indent-top-level-change-count')
  ...
  ) return rejected('nested-nonempty-bullet-indent-list-shape')

  const candidates = []
  for (let targetIndex = 1; targetIndex < previousList.childCount; targetIndex += 1) {
    ...
    if (
      !plainBulletItem(target) || target.childCount !== 1 || !plainTextParagraph(target.firstChild) ||
    ...
    for (let oldIndex = 0; oldIndex < previousList.childCount; oldIndex += 1) {
      if (oldIndex === parentIndex || oldIndex === targetIndex) continue
    ...
    if (siblingsMatch) {
      candidates.push({ targetIndex, parentIndex, target, previousParent, nextParent })
    }
  }
  if (candidates.length !== 1) {
    return rejected('nested-nonempty-bullet-indent-target-count', {
  ...
  if (journal.transactionCount !== 1 || journal.stepCount !== 1 || journal.entries.length !== 1) {
    return recognizedRejection('nested-nonempty-bullet-indent-transaction-count')
  }
  ...
  ) return recognizedRejection('nested-nonempty-bullet-indent-step-document')
  ...
  ) return recognizedRejection('nested-nonempty-bullet-indent-step-shape')

  const parentPath = [topLevelIndex, target.parentIndex]
  const targetPath = [topLevelIndex, target.targetIndex]
  ...
  ) return recognizedRejection('nested-nonempty-bullet-indent-step-slice')
  ...
  ) return recognizedRejection('nested-nonempty-bullet-indent-step-range')
  ...
  if (applied?.failed || !applied?.doc || !sameSourceSyncDocument(applied.doc, expectedDoc)) {
    return recognizedRejection('nested-nonempty-bullet-indent-step-result')
  }
  ...
  return Object.freeze({
    ...
    nestedListPath: Object.freeze([topLevelIndex, target.parentIndex, 1]),
    nestedItemPath: Object.freeze([topLevelIndex, target.parentIndex, 1, 0]),
    parentText: target.previousParent.firstChild.textContent,
    targetText: target.target.firstChild.textContent,
    position: target.targetIndex === previousList.childCount - 1 ? 'tail' : 'middle',
    step: Object.freeze({
...
const resolveSourceList = ({ markdown, doc, topLevelIndex, parentIndex, resolveMarkdownOffset }) => {
  ...
  let rawOffset
  try {
    rawOffset = resolveMarkdownOffset({ markdown, pmPos: paragraph.contentStart, doc, topLevelIndex })
  } catch {
    return null
  }
...
const patchAuthoredIndent = ({ source, sourceList, classification }) => {
  ...
  if (parent.body !== classification.parentText || target.body !== classification.targetText) return null
  ...
  return Object.freeze({
    ...
    row: Object.freeze({ token: target.token, spacing: target.spacing, body: target.body, indent: target.indent }),
    parentRow: Object.freeze({ token: parent.token, spacing: parent.spacing, body: parent.body, indent: parent.indent })
  })
}
...
const createOwnedPlan = ({ boundary, markdown, canonical, expectedDoc, proof }) => {
  const result = Object.freeze({
    markdown,
    preserved: true,
    reason: 'list-nested-nonempty-bullet-indented',
  ...
  return Object.freeze({
    ...
    family: LIST_NESTED_NONEMPTY_BULLET_INDENT_TRANSACTION_FAMILY,
    boundary,
...
export function createListNestedNonemptyBulletIndentTransactionSourceSyncOwner({
  resolveMarkdownOffset,
  validateMarkdown
} = {}) {
  if (typeof resolveMarkdownOffset !== 'function') {
    throw new TypeError('nested nonempty bullet indent owner requires resolveMarkdownOffset')
  }
  if (typeof validateMarkdown !== 'function') {
    throw new TypeError('nested nonempty bullet indent owner requires validateMarkdown')
  }
  ...
  const plan = ({
    ...
    boundary = LIST_NESTED_NONEMPTY_BULLET_INDENT_TRANSACTION_BOUNDARY
  } = {}) => {
    if (!journal || activeJournal !== journal) {
      return rejected('nested-nonempty-bullet-indent-journal-stale', { reset: true })
    }
    ...
    ) return rejected('nested-nonempty-bullet-indent-plan-incomplete', { reset: true })
    if (currentSource !== snapshot.source || currentCanonical !== snapshot.canonical) {
      return rejected('nested-nonempty-bullet-indent-live-snapshot-stale', { reset: true })
    }
    if (callbackDocumentEquivalent !== true) {
      return rejected('nested-nonempty-bullet-indent-callback-document-mismatch', { deferred: true })
    }
    ...
    if (!sourceList) return recognizedRejection('nested-nonempty-bullet-indent-range-unmapped')

    const patched = patchAuthoredIndent({ source: journal.source, sourceList, classification })
    if (!patched) return recognizedRejection('nested-nonempty-bullet-indent-source-row-unproven')

    let valid = false
    try { valid = validateMarkdown({ markdown: patched.markdown, expectedDoc }) === true } catch { valid = false }
    if (!valid) return recognizedRejection('nested-nonempty-bullet-indent-source-invalid')

    const proof = Object.freeze({
      kind: 'transaction-list-nested-nonempty-bullet-indent-proof',
